...
const Pagination: FC<PaginationProps> = ({ onPageChangeRequested }) => {
    ...
    return <div className={css['pagination']}>
        {Array(totalPages).fill(0).map((_, i) => i + 1).map(page => {
            if (page === currentPage) {
                return <span className={css['page']}><b>{page}</b></span>
            }

            return <span onClick={() => onPageChangeRequested(page)} className={css['page']}>{page}</span>;
        })}
    </div>
}
