...
const ListingPage: FC = () => {
    ...
    const { fetchMovies, resultOriginCount, isSearchInProgress, movies } = useMoviesContext();
    ...
    const handleKeyPressedOnInput = (...args: Parameters<KeyboardEventHandler<HTMLInputElement>>) => {
        if (args[0].key === "Enter") {
    ...
    const listFramgent = useMemo(() => {
        if (isSearchInProgress) {
            return <div className={css["spinner-area"]}><span className={css["spinner"]} /></div >
        }
        return (
            <>
    ...
    }, [isSearchInProgress, fetchMovies, movieName]);
    ...
    return (
        <div className={css['layout']}>
            ...
            <div className={css['movieSearch']}>
                <TextInput size={TextInputSize.SMALL} value={movieName} setValue={handleMovieNameChange} placeHolder="Enter movie name" onKeyDown={handleKeyPressedOnInput} />
